...
const InfluencerListingCard = ({data}) => {
  return (
    <>
      {/* Mobile view */}
      <div className="md:hidden w-full px-2 py-4">
        <div className="bg-slate-50 p-4 rounded-lg shadow-md space-y-3 max-w-xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-lg font-bold text-gray-800"><Link to="/Influencer/listing/detail/page" state={{ listingData: data }}>{data.name}</Link></p>
]          </div>
          <div className="flex items-center gap-3">
            <img src="/Location.jpg" alt="Location Icon" className="w-5 h-5 object-contain" />
            <p className="text-sm text-gray-700 font-medium"> {data.city} {data.pincode}  {data.state} {data.country}</p>
          </div>
          ...
          <div className='flex  flex-col font-extrabold text-zinc-600 font-mono'>
            <h1 >Facebook Follower :{data.fbFollower}</h1>
            <h1 className='content-end'>Instagram Follower:{data.instagramFollower} </h1>
            </div>
          </div>
      </div>
      ...
      <div className="w-full h-64 hidden md:flex bg-slate-50 p-4 space-x-4 mb-4 rounded-lg">
        ...
        <div className="w-5/12 flex flex-col justify-start items-start space-y-3">
          <div className="flex items-center gap-3">
            ...
            <p className="text-xl font-bold text-gray-800"><Link to="/Influencer/listing/detail/page" state={{ listingData: data }}>{data.name}</Link></p>
          </div>
          <div className="flex items-center gap-3">
