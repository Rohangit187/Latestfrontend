...
function PropertyNavbar() {
  const {isAuthenticated, loginWithRedirect } = useAuth0();
  const navigate = useNavigate();

  const handleClick = async () => {
    if (isAuthenticated) {
    navigate('/Property/create/listing');
    } else {
      await loginWithRedirect({
        appState: { returnTo: '/Property/Homepage' }
      });
    }
  };
...
>
  ...
  <div className="navbar-center hidden lg:block bg-gray-50">
    ...
    <li><a className="hover:text-primary transition-all duration-200">PG-hostel</a></li>
    </ul>
  </div>
